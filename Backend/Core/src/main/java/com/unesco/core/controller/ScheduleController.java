...
//----------------------------------------------------------------------------------------------------------------------
        EducationPeriodEntity period = pairDataService.getEducationPeriodForYearAndSemester(
                pairModel.getLesson().getSemesterNumberYear().getSemester(),
                pairModel.getLesson().getSemesterNumberYear().getYear());
        ...
        if (findLesson == null) {
            LessonDTO lesson = pairModel.getLesson();
            lesson.setId(0);
            pairModel.setLesson(lesson);
            ResponseStatusDTO<LessonDTO> saveLessonStatus = lessonDataService.save(pairModel.getLesson());
        ...
        //--------------------------------------------------------------------------------------------------------------
        if (!skipWarnings) {
            result = pairManager.CheckIntersection(pairDataService.getAllWhereYearAndSemesterFit(findLesson.getId(),period.getId()));
            if (result.getStatus() != StatusTypes.OK) return result;
        }

        result = pairDataService.save(pairManager.get(),(EducationPeriodDTO) mapperService.toDto(period),findLesson);
