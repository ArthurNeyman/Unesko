...
public class PairDataService implements IPairDataService {
    ...
    public List<PairDTO> getAllWhereYearAndSemesterFit(long lessonId, long periodId) {
        List<PairDTO> modelList = new ArrayList<>();

        Iterable<PairEntity> entityList = pairRepository.findPairsByPeriodId(periodId);
    ...
    public EducationPeriodEntity getEducationPeriodForYearAndSemester(int semester, int year) {
    ...
    public ResponseStatusDTO<PairDTO> save(PairDTO pair, EducationPeriodDTO period,LessonDTO findLesson) {
        PairEntity entity = (PairEntity) mapperService.toEntity(pair);
        ...
        try {
            entity = pairRepository.save(entity);
            LessonEntity lesson = entity.getLesson();
            lesson.setEducationPeriod((EducationPeriodEntity)mapperService.toEntity(period) );
            entity.setLesson(lesson);
        } catch (Exception e) {
            result.setStatus(StatusTypes.ERROR);
            result.addErrors(e.getMessage());
